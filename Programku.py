...
if xgb_model is not None and rf_model is not None and xgb_scaler is not None and rf_scaler is not None:
    ...
    input_data['TN_Bgr'] = st.sidebar.number_input("Suhu Min Bogor (°C)", min_value=0.0, value=19.6, format="%.1f")
    input_data['TX_Bgr'] = st.sidebar.number_input("Suhu Max Bogor (°C)", min_value=0.0, value=25.8, format="%.1f")
    input_data['TAVG_Bgr'] = st.sidebar.number_input("Suhu Rata-rata Bogor (°C)", min_value=0.0, value=22.2, format="%.1f")
    input_data['RH_AVG_Bgr'] = st.sidebar.number_input("Kelembaban Rata-rata Bogor (%)", min_value=0.0, value=92.0, format="%.1f")
    input_data['RR_Bgr'] = st.sidebar.number_input("Curah Hujan Bogor (mm)", min_value=0.0, value=23.5, format="%.1f")

    st.sidebar.subheader("Data Cuaca Jakarta")
    input_data['TN_Jkt'] = st.sidebar.number_input("Suhu Min Jakarta (°C)", min_value=0.0, value=26.4, format="%.1f")
    input_data['TX_Jkt'] = st.sidebar.number_input("Suhu Max Jakarta (°C)", min_value=0.0, value=32.2, format="%.1f")
    input_data['TAVG_Jkt'] = st.sidebar.number_input("Suhu Rata-rata Jakarta (°C)", min_value=0.0, value=29.6, format="%.1f")
    input_data['RH_AVG_Jkt'] = st.sidebar.number_input("Kelembaban Rata-rata Jakarta (%)", min_value=0.0, value=77.0, format="%.1f")
    input_data['RR_Jkt'] = st.sidebar.number_input("Curah Hujan Jakarta (mm)", min_value=0.0, value=1.8, format="%.1f")
